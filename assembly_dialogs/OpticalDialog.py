...
class OpticalDialog (AbstractAssemblyDialog):
    ...
    def go_to_guide(self, barcode):
        guide = [
            {"text": f"connect optics using mfb {self.geo_data['mfb']} in channel {self.geo_data['opt_services_channel']}", "image": 'assembly_images/test_1.png'},
            {"text": "test 1", "image": None},
            {"text": "test 2", "image": 'assembly_images/test_2.png'},
            {"text": "test 3", "image": 'assembly_images/test_3.png'}
        ]
        title = f"Connecting module with barcode {self.module.barcode} to optics at {self.geo_data['opt_services_channel']}"
    ...
    def proceed(self):
        self.module.status.opt_status = func.now()
